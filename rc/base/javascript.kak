...
define-command -hidden init-javascript-filetype -params 1 %~
    ...
    add-highlighter shared/ regions -default code %arg{1} \
        ...
        comment       /\*  \*/                     '' \
        regex         /    (?<!\\)(\\\\)*/[gimuy]* '' \
        jsx           (?<![\w<])<[a-zA-Z][a-zA-Z0-9:.-]* (</.*?>|/>) (?<![\w<])<[a-zA-Z][a-zA-Z0-9:.-]* \
    ...
    add-highlighter "shared/%arg{1}/jsx/content/comment" fill comment
    ...
    add-highlighter "shared/%arg{1}/code" regex \b(async|await|break|case|catch|class|const|continue|debugger|default|delete|do|else|export|extends|finally|for|function|if|import|in|instanceof|let|new|of|return|super|switch|throw|try|typeof|var|void|while|with|yield)\b 0:keyword
    ...
    hook global WinSetOption "filetype=%arg{1}" "
        hook window InsertEnd  .* -group %arg{1}-hooks  javascript-filter-around-selections
